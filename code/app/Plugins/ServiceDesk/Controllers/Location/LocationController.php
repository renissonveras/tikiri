...
class LocationController extends BaseServiceDeskController {
        ...
        try {
            ...
            return \Datatable::Collection($locationcategory)
                            ...
                            ->addColumn('action', function($model) {
                                return "<a href=" . url('service-desk/location-types/' . $model->id . '/edit') . " class='btn btn-info btn-sm'>Editar</a> <a href=" . url('service-desk/location-types/' . $model->id . '/show') . " class='btn btn-primary btn-sm'>Ver</a>";
                            })
    ...
    public function handleCreate(CreateLocationRequest $request) {
        ...
        try {
            ...
            $sd_location->save();
            return \Redirect::route('service-desk.location.index')->with('message', Lang::get('service::lang.sucess_on_operation'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
    ...
    public function handledelete($id) {
        try {
            $sd_location = SdLocations::findOrFail($id);
            $sd_location->delete();
            return \Redirect::route('service-desk.location-category.index')->with('message', Lang::get('service::lang.sucess_on_operation'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
