...
class AssetController extends BaseServiceDeskController {
		...
		try {
			...
			return \Datatable::Collection($assets)
							...
							->addColumn('action', function($model) {
								...
								return "<a href=" . url('service-desk/assets/' . $model->id . '/edit') . " class='btn btn-info btn-sm'>Editar</a> "
										. $delete
										. " <a href=" . url('service-desk/assets/' . $model->id . '/show') . " class='btn btn-primary btn-sm'>Ver</a>";
							})
	...
	public function handleEdit($id, CreateAssetRequest $request) {
		try {
			...
			$form = $request->except('organization', 'external_id', 'product_id', '_token', 'name', 'description', 'department_id', 'asset_type_id', 'impact_type_id', 'managed_by', 'used_by', 'location_id', 'assigned_on');
			$this->storeAssetForm($sd_assets->id, $form);
			$result = ["success" => "Ativo $sd_assets->name atualizado com sucesso"];
			return response()->json(compact('result'));
	...
	public function assetHandledelete($id) {
		try {
			...
			return \Redirect::route('service-desk.asset.index')->with('message', 'Ativo excluído com sucesso !!!');
		} catch (Exception $ex) {
			return redirect()->back()->with('fails', $ex->getMessage());
		}
	}
	...
	public function attachAssetToTicket(Request $request) {
		try {
			...
			//if ($relation) {
			return redirect()->back()->with('success', 'Ativo adicionado com sucesso');
			//}
		} catch (Exception $ex) {
			dd($ex);
			return redirect()->back()->with('fails', $ex->getMessage());
	...
	public function assetType(Request $request) {
		try {
			...
			return \Datatable::table()
							->addColumn('#', 'Ativos', 'Utilizado por')
	...
	public function marbleHtml($ticketid, $asset_name, $user_name, $managed_by, $assetid) {
		...
		return "<div class='box box-primary'>"
				...
				. "<h3 class='box-title'>Ativos Associados</h3>"
				. "</div>"
				...
				. "<th><i>Utilizado por: </i> " . ucfirst($user_name) . "</th>"
				. "<th><i>Gerenciado por: </i> " . ucfirst($managed_by) . "</th>"
				. "<th>" . $detach_popup
				. " | <a href=" . url('service-desk/assets/' . $assetid . '/show/') . ">Ver</a></th>"
				. "</table>"
				. "</div>"
				. "</div>"
				. "</div>";
