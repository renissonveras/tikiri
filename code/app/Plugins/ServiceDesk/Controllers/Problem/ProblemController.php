...
class ProblemController extends BaseServiceDeskController {
        ...
        try {
            ...
            return \Datatable::Collection($problems)
                            ...
                            ->addColumn('Action', function($model) {
                                ...
                                $delete = \App\Plugins\ServiceDesk\Controllers\Library\UtilityController::deletePopUp($model->id, $url, "Excluir $model->subject");
                                return "<a href=" . url('service-desk/problem/' . $model->id . '/edit') . " class='btn btn-info btn-sm'>Editar</a> "
                                        . $delete
                                        . " <a href=" . url('service-desk/problem/' . $model->id . '/show') . " class='btn btn-primary btn-sm'>Ver</a>";
                            })
    ...
    public function handleCreate(CreateProblemRequest $request) {
        ...
        try {
            $this->store($request);
            return \Redirect::route('service-desk.problem.index')->with('success', Lang::get('service::lang.sucess_on_operation'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
...
// dd($id);  
    ...
    public function attachNewProblemToTicket(Request $request) {
        try {
            ...
            if ($store) {
                ...
                return redirect()->back()->with('success', 'Foi criado um novo problema associado a este ticket');
            }
            return redirect()->back()->with('fails', 'Desculpa! Não foi possível processar a requisição');
        } catch (\Exception $ex) {
            dd($ex);
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    ...
    public function attachExistingProblemToTicket(Request $request) {
        try {
            ...
            return redirect()->back()->with('success', 'Problema associado a este ticket');
        } catch (Exception $ex) {
            dd($ex);
        }
    }
    ...
    public function marbleHtml($ticketid, $problemid, $subject, $content) {
        ...
        return "<div class='box box-primary'>"
                ...
                . "<h3 class='box-title'>Problemas Associados</h3>"
                . "</div>"
                ...
                . "<th>".$detach_popup
                . "  | <a href=" . url('service-desk/problem/' . $problemid . '/show') . ">Ver</a></th>"
                . "</table>"
                . "</div>"
                . "</div>"
                . "</div>";
    ...
    public function detach($ticketid) {
        ...
        return redirect()->back()->with('success', 'Desvinculado com sucesso');
    }
