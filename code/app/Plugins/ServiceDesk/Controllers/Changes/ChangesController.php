...
class ChangesController extends BaseServiceDeskController {
        ...
        try {
            ...
            return \Datatable::Collection($changes)
                            ...
                            ->addColumn('action', function($model) {
                                ...
                                //dd($delete);
                                return "<a href=" . url('service-desk/changes/' . $model->id . '/edit') . " class='btn btn-info btn-sm'>Editar</a> "
                                        . $delete
                                        . " <a href=" . url('service-desk/changes/' . $model->id . '/show') . " class='btn btn-primary btn-sm'>Ver</a>";
                            })
    ...
    public function changeshandleCreate(CreateChangesRequest $request, $attach = false) {
        ...
        try {
            ...
            if ($attach == false) {
                return \Redirect::route('service-desk.changes.index')->with('message', Lang::get('service::lang.sucess_on_operation'));
            }
            return $sd_changes;
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
