...
class RelesesController extends BaseServiceDeskController {
        ...
        try {
            ...
            return \Datatable::Collection($relese)
                            ...
                            ->addColumn('Action', function($model) {
                                ...
                                return "<a href=" . url('service-desk/releases/' . $model->id . '/edit') . " class='btn btn-info btn-sm'>Editar</a> "
                                        . $delete
                                        . " <a href=" . url('service-desk/releases/' . $model->id . '/show') . " class='btn btn-primary btn-sm'>Ver</a>";
                             })
    ...
    public function releaseshandleCreate(CreateReleaseRequest $request,$return=false) {
        ...
        try {
            ...
            $sd_releases->planned_start_date = $request->planned_start_date;
            $sd_releases->planned_end_date = $request->planned_end_date;
            ...
            if($return===false){
                return \Redirect::route('service-desk.releases.index')->with('message', Lang::get('service::lang.sucess_on_operation'));
            }
            return $sd_releases;
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
    ...
    public function releaseshandleEdit($id,CreateReleaseRequest $request) {
        try {
            ...
            echo $request->planned_start_date = date('Y-m-d H:i:s', strtotime($request->planned_start_date));
            $request->planned_end_date = date('Y-m-d H:i:s', strtotime($request->planned_end_date));
