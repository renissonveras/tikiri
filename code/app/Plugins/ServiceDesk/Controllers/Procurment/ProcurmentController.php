...
class ProcurmentController extends BaseServiceDeskController {
        ...
        try {
            ...
            return \Datatable::Collection($procurments)
                            ...
                            ->addColumn('Action', function($model) {
                                return "<a href=" . url('service-desk/procurement/' . $model->id . '/edit') . " class='btn btn-info btn-xs'>Editar</a> ";
    ...
    public function handleCreate(CreateProcurmentRequest $request) {
        try {
            ...
            return \Redirect::route('service-desk.procurment.index')->with('message', Lang::get('service::lang.sucess_on_operation'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
