...
class LicensetypeController extends BaseServiceDeskController {
        ...
        try {
            ...
            return \Datatable::Collection($license_type)
                            ...
                            ->addColumn('action', function($model) {
                                return "<a href=" . url('service-desk/license-types/' . $model->id . '/edit') . " class='btn btn-info btn-xs'>Editar</a>";
                            })
    ...
    public function handleCreate(CreateLicensetypesRequest $request) {
        try {
            ...
            return \Redirect::route('service-desk.licensetypes.index')->with('message', Lang::get('service::lang.sucess_on_operation'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
