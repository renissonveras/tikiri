...
class ProductsController extends BaseServiceDeskController {
        ...
        try {
            ...
            return \Datatable::Collection($products)
                            ...
                            ->addColumn('action', function($model) {
                                return "<a href=" . url('service-desk/products/' . $model->id . '/edit') . " class='btn btn-info btn-sm'>Editar</a> <a href=" . url('service-desk/products/' . $model->id . '/show') . " class='btn btn-primary btn-sm'>Ver</a>";
                            })
    ...
    public function productshandleCreate(CreateProductsRequest $request) {
        try {
            ...
            $sd_products->save();
            return \Redirect::route('service-desk.products.index')->with('message', Lang::get('service::lang.sucess_on_operation'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
    ...
    public function addVendor(CreateVendorRequest $request) {
        try {
            ...
            return redirect()->back()->with('fails', 'Não foi possível processar');
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
