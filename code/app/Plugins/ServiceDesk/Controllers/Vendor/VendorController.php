...
class VendorController extends BaseServiceDeskController {
        ...
        try {
            ...
            return \Datatable::Collection($vandor)
                            ...
                            ->addColumn('Action', function($model) {
                                ...
                                return "<a href=" . url('service-desk/vendor/' . $model->id . '/edit') . " class='btn btn-info btn-sm'>Editar</a> "
                                        . $delete
                                        . " <a href=" . url('service-desk/vendor/' . $model->id . '/show') . " class='btn btn-primary btn-sm'>Ver</a>";

                                //return "<a href=" . url('service-desk/vendor/' . $model->id . '/edit') . " class='btn btn-info btn-xs'>Editar</a> <a href=" . url('service-desk/vendor/' . $model->id . '/delete') . " class='btn btn-warning btn-xs btn-flat'>Delete</a>";
                            })
    ...
    public function handleCreate(CreateVendorRequest $request, $return = false) {
        try {
            ...
            $sd_vendors->save();
            if ($return == false) {
                return \Redirect::route('service-desk.vendor.index')->with('message', \Lang::get('service::lang.sucess_on_operation'));
            }
            return $sd_vendors;
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
