...
class CabController extends BaseServiceDeskController {
    ...
    public function getCab() {
        ...
        return Datatable::Collection($cabs->select('name', 'head', 'id')->get())
                        ...
                        ->addColumn('action', function($model) {
                            return "<a href=" . url('service-desk/cabs/' . $model->id . '/edit') . " class='btn btn-info btn-xs'>Editar</a>";
                        })
    ...
    public function vote($cabid, $owner) {
        try {
            ...
            if (!$check) {
                return redirect('/service-desk/problems')->with('fails', 'Não autorizado');
            }
            $authid = \Auth::user()->id;
            $votes = new CabVote();
            $vote = $votes->where('cab_id', $cabid)->where('user_id', $authid)->where('owner', $owner)->first();
            if ($vote) {
                return redirect('/service-desk/problems')->with('fails', 'Já votou');
            }
    ...
    public function postVote($cabid, $owner, Request $request) {
        try {
            ...
            return redirect('/service-desk/problems')->with('success', 'Voto realizado com sucesso');
        } catch (Exception $ex) {
            return redirect('/service-desk/problems')->with('fails', $ex->getMessage());
        }
    }
    ...
    public static function checkVote($vote){
        $value = "";
        if($vote==1){
            $value = "Votado para proceder";
        }else{
            $value = "Votado para não proceder";
        }
        return $value;
    }
