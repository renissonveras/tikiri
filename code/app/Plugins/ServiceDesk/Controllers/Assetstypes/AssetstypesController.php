...
class AssetstypesController extends BaseServiceDeskController {
        ...
        try {
            ...
            return \Datatable::Collection($assets)
                            ...
                            ->addColumn('action', function($model) {
                                return "<a href=" . url('service-desk/assetstypes/' . $model->id . '/edit') . " class='btn btn-info btn-xs'>Editar</a> ";
                            })
    ...
    public function handleCreate(CreateAssetstypesRequest $request) {
        try {
            ...
            return \Redirect::route('service-desk.assetstypes.index')->with('message', \Lang::get('service::lang.sucess_on_operation'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
