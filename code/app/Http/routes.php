...
Route::group(['middleware' => ['web']], function () {
    ...
    Breadcrumbs::register('password.email', function ($breadcrumbs) {
        ...
        $breadcrumbs->push('Senha Perdida', url('password/email'));
    });
    ...
    Breadcrumbs::register('auth.register', function ($breadcrumbs) {
        $breadcrumbs->parent('/');
        $breadcrumbs->push('Login', url('auth/login'));
        $breadcrumbs->push('Registrar-se', url('auth/register'));
    });
  ...
  // Auth login
    ...
    Breadcrumbs::register('auth.login', function ($breadcrumbs) {
        $breadcrumbs->parent('/');
        $breadcrumbs->push('Autenticação', url('auth/register'));
        $breadcrumbs->push('Login', url('auth/login'));
    });
...
//    Route::post('org-chart-range', ['as' => 'post.org.chart', 'uses' => 'Agent\helpdesk\OrganizationController@orgChartData']);
        ...
        Route::patch('/thread/{id}', ['as' => 'ticket.thread', 'uses' => 'Agent\helpdesk\TicketController@thread']); /*  Get Thread by ID */
        ...
        Route::post('ratingArticle/{id}', ['as' => 'rating.article', 'uses' => 'Client\kb\UserController@ratingArticle']); /* Get reply Ratings */
...
//    });
    ...
    Route::get('ajax-department/{id}', 'Client\helpdesk\FormController@selectDepartmentAjax');
...
//    Route::get('/reportdata', 'HomeController@pushdata');
    ...
    Route::get('get-latest-code', ['as' => 'file.lastcode', 'uses' => 'Update\UpgradeController@getLatestVersion']);
    /*
     * Webhook
     */
