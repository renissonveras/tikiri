...
class CheckUpdate
{
    ...
    {
        // Desabilitado nas primeiras versões do Tikiri
        //$check = $this->process();
...
//            }
    ...
    public function notificationBar()
    {
        ...
        if (is_dir($path)) {
            $notify->create(['key' => 'update-ready', 'value' => 'Nova versão disponível, <a href='.url('file-update').'>clique aqui</a> para atualizar agora.']);
        }
    }
    ...
    public function checkNewUpdate()
    {
        ...
        if ($not->count() > 0) {
            ...
            if (count($notifications) > 0) {
                if (!array_key_exists('new-version', $notifications)) {
                    ...
                    if ($check_version == true) {
                        $notify->create(['key' => 'new-version', 'value' => 'Nova versão encontrada! <a href='.url('file-update').'><b>Clique aqui para baixar.</b></a>']);
                    }
                } else {
            ...
            if ($check_version == true) {
                //dd('if');
                $notify->create(['key' => 'new-version', 'value' => 'Nova versão encontrada! <a href='.url('file-update').'><b>Clique aqui para baixar.</b></a>', 'created_at' => \Carbon\Carbon::now()]);
            } else {
                //dd('else');
