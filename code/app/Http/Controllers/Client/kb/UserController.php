...
class UserController extends Controller
{
    ...
    /**
     * Store Client rating about an article quality.
     *
     * @return type json
     */
    public function ratingArticle($id, Request $request, \App\Model\helpdesk\Ratings\RatingRef $rating_ref)
    {
        try{        
            foreach ($request->all() as $key => $value) {
                ...
                $ratingrefs = $rating_ref->where('rating_id', '=', $ratID->id)->where('article_id', '=', $id)->first();
                if ($ratingrefs !== null) {
                    $ratingrefs->rating_id = $ratID->id;
                    $ratingrefs->article_id = $id;
                    $ratingrefs->ticket_id = '0';
                    ...
                    $rating_ref->article_id = $id;
                    $rating_ref->ticket_id = '0';
                    ...
                    $data = [
                        'rating_value' => $value,
                        'assigned_to'  => $articles->autor_id,
                    ];
                    \Event::fire('Article-Rating', [$data]);
                }
            }
            //return $request->all();
