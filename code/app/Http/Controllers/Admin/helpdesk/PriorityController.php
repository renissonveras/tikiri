...
class PriorityController extends Controller
{
    ...
    public function userPriorityIndex(Request $request)
    {
        try {
            ...
            return 'Seu status foi atualizado';
        } catch (Exception $e) {
        ...
        try {
            ...
            return \Datatable::Collection($tickets)
                            ...
                            ->addColumn('status', function ($model) {
                                if ($model->status == 1) {
                                    return "<a style='color:green'>ativo</a>";
                                } elseif ($model->status == 0) {
                                    Ticket_Priority::where('priority_id', '=', '$priority_id')
                                    ->update(['priority_id' => '']);

                                    return "<a style='color:red'>inativo</a>";
                                }
                            })
                            ->addColumn('action', function ($model) {
                                if ($model->is_default > 0) {
                                    return '<a href='.url('ticket/priority/'.$model->priority_id.'/edit')." class='btn btn-info btn-xs' disabled='disabled'>Editar</a>&nbsp;<a href=".url('ticket/priority/'.$model->priority_id.'/destroy')." class='btn btn-warning btn-info btn-xs' disabled='disabled' > Excluir </a>";
                                } else {
                                    return '<a href='.url('ticket/priority/'.$model->priority_id.'/edit')." class='btn btn-info btn-xs'>Editar</a>&nbsp;<a class='btn btn-danger btn-xs' onclick='confirmDelete(".$model->priority_id.")'>Excluir </a>";
                                }
                            })
