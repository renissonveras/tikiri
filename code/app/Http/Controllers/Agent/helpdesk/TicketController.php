...
//            dd($tickets);
	...
	public function size()
	{
		...
		if (!$files) {
			throw new \Exception('tamanho do arquivo excedido');
		}
		$size = 0;
		if (count($files) > 0) {
	...
	public function reply(Ticket_Thread $thread, Request $request, Ticket_attachments $ta, $mail = true, $system_reply = true, $user_id = '')
	{
		...
		try {
			...
			if ($system_reply == true) {
				if ($tickets->assigned_to == 0) {
					...
					$thread2->body = 'Este ticket foi atribuido a '.Auth::user()->first_name.' '.Auth::user()->last_name;
					$thread2->save();
					$data = [
						'id' => $tickets->id,
					];
		...
		$result = ['success' => 'Respondido com Sucesso'];
...
//                Event fire for new ticket['']
	...
	public function close($id, Tickets $ticket)
	{
		...
		return 'Seu ticket'.$ticket_status->ticket_number.' foi fechado com sucesso.';
	}
...
//        $ticket_status = $ticket->where('id', '=', $id)->first();
		...
		return 'Seu ticket'.$ticket_status->ticket_number.' foi resolvido com sucesso.';
	}
	...
	public function open($id, Tickets $ticket)
	{
		...
		return 'seu ticket'.$ticket_status->ticket_number.' foi aberto.';
	}
	...
	public function delete($id, Tickets $ticket)
	{
		...
		if ($ticket_delete->status == 5) {
			...
			return 'seu ticket foi excluído';
		} else {
			...
			return 'seu ticket'.$ticket_delete->ticket_number.' foi excluído';
		}
	}
	...
	public function assign($id)
	{
		...
		foreach ($ticket_array as $id) {
			...
			if ($assign_to[0] == 'team') {
				...
				$thread->user_id = Auth::user()->id;
				$thread->is_internal = 1;
				$thread->body = 'Este ticket foi atribuido a '.$assignee;
				$thread->save();
	...
	{
		$ticket = Tickets::where('id', '=', $id)->first();
		$InternalContent = Auth::user()->first_name.' '.Auth::user()->last_name.' retirou a atribuição dada a um técnico para este ticket';
		$thread = Ticket_Thread::where('ticket_id', '=', $id)->first();
		$NewThread = new Ticket_Thread();
		$NewThread->ticket_id = $thread->ticket_id;
		$NewThread->user_id = Auth::user()->id;
	...
	public function usersearch()
	{
		...
		if (!isset($ticket_collaborator)) {
			...
			return '<div id="alert11" class="alert alert-dismissable" style="color:#60B23C;background-color:#F2F2F2;"><button id="dismiss11" type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i>Sucesso!</h4><h4><i class="icon fa fa-user"></i>'.$data->user_name.'</h4><div id="message-success1">'.$data->email.'</div></div>';
		} else {
			return '<div id="alert11" class="alert alert-warning alert-dismissable"><button id="dismiss11" type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-warning"></i>'.$data->user_name.'</h4><div id="message-success1">'.$data->email.'<br/>Este usuário já colaborou</div></div>';
		}
	}
	...
	public function useradd()
	{
		...
		if (isset($user_serach)) {
			return '<div id="alert11" class="alert alert-warning alert-dismissable" ><button id="dismiss11" type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-alert"></i>Alert!</h4><div id="message-success1">Este usuário já existe</div></div>';
		} else {
			$company = $this->company();
			$user = new User();
...
//                        dd($thread);
		...
		return redirect()->back()->with('fails', 'Nada selecionado!');
	}
	...
	public function deptopen($id)
	{
		...
		if (Auth::user()->role == 'agent') {
			if (Auth::user()->primary_dpt == $dept->id) {
				...
				return redirect()->back()->with('fails', 'Não Autorizado!');
			}
		} else {
			return view('themes.default1.agent.helpdesk.dept-ticket.tickets', compact('id'));
		}
		...
		try{
			foreach ($request->all() as $key => $value) {
				...
				if ($ratingrefs !== null) {
					...
					$rating_ref->save();
					$tickets = Tickets::find($id)->first();
					$data = [
						'rating_value' => $value,
						'assigned_to'       => $tickets->assigned_to,
					];
					\Event::fire('Ticket-Rating', [$data]);					
				}
			}
			
			 return response()->json(['success' => 'Obrigado por avaliar!']);
		} catch (Exception $ex) {
			return $ex->getMessage();
		}        
	}
	...
	public function changeOwner($id)
	{
		...
		if ($count === 1) {
			...
			$thread->user_id = Auth::user()->id;
			$thread->is_internal = 1;
			$thread->body = 'O ticket agora pertence a '.$user->user_name;
			$thread->save();
		...
		return \Datatables::of($tickets)
			...
			->addColumn('title', function ($tickets) {
				...
				return "<div class='tooltip1' id='tool".$tickets->id."'>
							...
							"<span class='tooltiptext'  id='tooltip".$tickets->id."'>Carregando...</span></div>";
			})
