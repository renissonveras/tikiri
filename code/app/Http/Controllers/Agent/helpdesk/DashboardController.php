...
class DashboardController extends Controller
{
    ...
    public function ChartData($date111 = '', $date122 = '')
    {
        ...
        for ($i = $date1; $i <= $date2; $i = $i + 86400) {
            $thisDate1 = date('Y-m-d', $i);

            $created = \DB::table('tickets')->select('created_at')->where('created_at', 'LIKE', '%'.$thisDate1.'%')->count();
            $closed = \DB::table('tickets')->select('closed_at')->where('closed_at', 'LIKE', '%'.$thisDate1.'%')->count();
            $reopened = \DB::table('tickets')->select('reopened_at')->where('reopened_at', 'LIKE', '%'.$thisDate1.'%')->count();

            $thisDate2 = date('d-m-Y', $i);
            $value = ['date' => $thisDate2, 'open' => $created, 'closed' => $closed, 'reopened' => $reopened];
    ...
    public function userChartData($id, $date111 = '', $date122 = '')
    {
        ...
        for ($i = $date1; $i <= $date2; $i = $i + 86400) {
            ...
            $thisDate2 = strtotime($thisDate);
            $value = ['date' => date('d-m-Y',$thisDate2), 'open' => $created, 'closed' => $closed, 'reopened' => $reopened];
            $array = array_map('htmlentities', $value);
            $json = html_entity_decode(json_encode($array));
            $return .= $json.',';
        }
