...
class SettingsController extends Controller
{
    ...
    public function edit_social_buttons($id, Widgets $widgets, Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', $widget->name.' Atualizado com sucesso');
        } catch (Exception $e) {
        ...
        return \Datatable::collection(new Collection($plugins))
                        ...
                        ->addColumn('name', function ($model) {
                            if (array_has($model, 'path')) {
                                if ($model['status'] == 0) {
                                    $activate = '<a href='.url('plugin/status/'.$model['path']).'>Ativar</a>';
                                    $settings = ' ';
                                } else {
                                    $settings = '<a href='.url($model['settings']).'>Configurações</a> | ';
                                    $activate = '<a href='.url('plugin/status/'.$model['path']).'>Desativar</a>';
                                }

                                $delete = '<a href="#"  id=delete'.$model['path'].' data-toggle=modal data-target=#del'.$model['path']."><span style='color:red'>Excluir</span></a>"
                                                    ...
                                                    <div class=modal-body>
                                                       <p>Tem certeza ?</p>
                                                        <div class=modal-footer>
                                                            ...
                                                            <a href='.url('plugin/delete/'.$model['path'])."><button class='btn btn-danger'>Excluir</button></a>
                                                        </div>
