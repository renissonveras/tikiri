...
class InstallController extends Controller
{
	...
	public function licencecheck(Request $request)
	{
		...
		if ($accept == 'accept') {
			...
			return Redirect::route('licence')->with('fails', 'Falhou! Você precisar aceitar o acordo de licença');
		}
	}
	...
	public function accountcheck(InstallerRequest $request)
	{
		...
		if (\Config::get('database.install') == '%0%') {
			...
			if (!$changed) {
				return \Redirect::back()->with('fails', 'Idioma Inválido');
			}
			// checking requested timezone for the admin and system
			$timezones = Timezones::where('name', '=', $timezone)->first();
			if ($timezones == null) {
				return redirect()->back()->with('fails', 'Time-zone inválido');
			}
			...
			if ($date_time_format == null) {
				return redirect()->back()->with('fails', 'Data/Hora inválido');
			}
			...
			return redirect()->back()->with('fails', 'Já existe um banco de dados configurado!');
		}
	...
	public function changeFilePermission()
	{
		...
		if (chmod($path1, 0644)) {
			...
			if ($f1 >= '644') {
				...
				return Redirect::back()->with('fail_to_change', 'Não foi possível alterar as permissões no seu servidor, tente manualmente.');
			}
		} else {
			return Redirect::back()->with('fail_to_change', 'Não foi possível alterar as permissões no seu servidor, tente manualmente.');
		}
