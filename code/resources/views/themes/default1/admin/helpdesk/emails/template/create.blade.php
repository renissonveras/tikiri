   ...
   {!! Form::submit(Lang::get('lang.save'),['class'=>'btn btn-primary'])!!}</div>
	 ...
	 <div class="box-body table-responsive no-padding"style="overflow:hidden">
		...
		<div class="form-group {{ $errors->has('template_set_to_clone') ? 'has-error' : '' }}">
			...
			{!!Form::select('template_set_to_clone', [''=>'Selecione um modelo','Templates'=>$templates->lists('name','name')],1,['class' => 'form-control']) !!}
			</div>
		</div>
		...
		<div class="form-group {{ $errors->has('language') ? 'has-error' : '' }}">
			...
			{!!Form::select('language', [''=>'Selecione a Linguagem','Languages'=>$languages->lists('name','name')],null,['class' => 'form-control']) !!}
			</div>
		</div>
