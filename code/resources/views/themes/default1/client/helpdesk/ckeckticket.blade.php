		...
		<div class="row">
			...
			<div class="col-md-3">
				<div class="pull-right">
					...
					<nav class="navbar navbar-transparent nav-thin">
				...
				<div class="ticketratings pull-right">    
					<table><tbody>
						...
						<form id="foo">
							...
							@if($rating->rating_area == 'Helpdesk')
							<?php
	...
	<ol class="comment-list" >
		<li class="comment">
			<article class="comment-body">
				<footer class="comment-meta"<?php if ($role->role == "user") { ?> style="background-color: #ffffff" <?php } else { ?> style="background-color:#eeeeee" <?php } ?>  > 
					<div class="comment-author">

						<img src="{{$role->profile_pic}}"alt="" height="50" width="50" class="avatar" <?php if ($role->role == "user") { ?>style="box-shadow: 0 1px 3px #333333;" <?php } else { ?> style="box-shadow: 0 1px 3px #FFEC00;" <?php } ?> >
						@if($role->role == "user")
						...
						<div class="ticketratings pull-right">   <table><tbody>
									@foreach($ratings as $rating) 
									@if($rating->rating_area == 'Comentarios')
									<?php
			...
			//alert(mydiv.height());
				...
				if (!!navigator.userAgent.match(/Trident\/7\./)){
					...
					jQuery('#loader_frame{{$conversation->id}}').css('height', h);
											...
											<script>
												 setTimeout(function(){ 
													   $('#loader_frame{{$conversation->id}}')[0].contentDocument.body.innerHTML = '<body><style>body{display:inline-block;font-family: "Roboto", "Helvetica", "Arial", sans-serif;font-weight: 300;font-size: 12px;}</style>{!!$conversation->purify()!!}<body>';   }, 1000);
				...
				<div class="timeline-footer" style="margin-bottom:-5px">
					...
					if ($i > 0) {
						echo "<hr style='border-top: 1px dotted #FFFFFF;margin-top:0px;margin-bottom:0px;background-color:#8B8C90;'><h4 class='box-title'><b>" . $i . " </b> Anexos</h4>";
					}
					?>
...
<div id="respond" class="card">
	<div class="header header-primary">
		<h3 id="reply-title" class="pad-side"><i class="material-icons">reply</i>{!! Lang::get('lang.leave_a_reply') !!}</h3>
	</div>
	@if(Auth::user()) 
	...
	<div class="content">
		<div class="form-group">
			<div class="col-md-12">
				<div class="form-group ">
					<textarea id="reply-input" class="texteditor" name="comment" cols="30" rows="8"></textarea>
				</div>
			</div>                                              
		</div>
	</div>
	<div class="footer text-right">
		<button type="submit" onClick="return checkFunction();" class="btn btn-primary btn-lg">{!! Lang::get('lang.post_comment') !!}</button>
	</div>
	{!! Form::close() !!}
</div>
	...
	$(document).ready(function () {
		...
		$('#foo').submit(function (event) {
			...
			$.ajax({
				...
				url: '../rating/' +{{ $tickets->id }}, // the url where we want to POST
		...
		$('.foo2').submit(function (event) {
			...
			$.ajax({
				headers: { 'X-CSRF-TOKEN' : '{{ csrf_token() }}' },				
				type: 'POST', // define the type of HTTP verb we want to use (POST for our form)
				url: '../rating2/' + {{ $tickets->id }}, // the url where we want to POST
				data: formData, // our data object
				dataType: 'json', // what type of data do we expect back from the server
				contentType: 'application/x-www-form-urlencoded',		
				success: function () {
		...
		$('#resolved').on('click', function (e) {
			$.ajax({
				...
				success: function (response) {
					...
					$("#refresh").show();
					$("#loader").hide();
					var message = "Sucesso! Seu ticket foi marcado como resolvido";
					$("#alert11").show();
					$('#message-success1').html(message);
					setInterval(function () {
						$("#alert11").hide();
		...
		$('#open').on('click', function (e) {
			$.ajax({
				...
				success: function (response) {
					...
					var message = "Sucesso! Seu ticket foi aberto";
					$("#alert11").show();
					$('#message-success1').html(message);
					setInterval(function () {
						$("#alert11").hide();
